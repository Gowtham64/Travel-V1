...
class _VehicleSearchSheetState extends State<VehicleSearchSheet> {
  ...
  void _showAddCustomVehicleDialog() {
    ...
    final mileageCtrl = TextEditingController(text: _selectedTypeFilter == 'motorcycle' ? '35.0' : '15.0');
    final tankCtrl = TextEditingController(text: _selectedTypeFilter == 'motorcycle' ? '13.0' : '45.0');
    String customType = _selectedTypeFilter == 'motorcycle' ? 'motorcycle' : 'car';
        ...
        builder: (context, setDlgState) {
          return AlertDialog(
            ...
            title: Row(
              children: [
                Icon(
                  customType == 'motorcycle' ? Icons.two_wheeler_rounded : Icons.directions_car_rounded,
                  color: const Color(0xFF38BDF8),
                  size: 24,
                ),
                const SizedBox(width: 10),
                const Text('Custom Vehicle / Bike', style: TextStyle(color: Colors.white, fontSize: 17, fontWeight: FontWeight.bold)),
              ],
            ),
            content: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  const Text('Vehicle Type', style: TextStyle(color: Colors.white70, fontSize: 12, fontWeight: FontWeight.w600)),
                  const SizedBox(height: 6),
                  Row(
                    children: [
                      Expanded(
                        child: ChoiceChip(
                          avatar: const Icon(Icons.two_wheeler_rounded, size: 16),
                          ...
                          onSelected: (v) {
                            if (v) {
                              setDlgState(() {
                                ...
                                if (mileageCtrl.text == '15.0') mileageCtrl.text = '35.0';
                                if (tankCtrl.text == '45.0') tankCtrl.text = '13.0';
                                if (customFuel == 'diesel') customFuel = 'petrol';
                              });
                            }
                          },
                        ),
                        ...
                        child: ChoiceChip(
                          ...
                          onSelected: (v) {
                            if (v) {
                              setDlgState(() {
                                customType = 'car';
                                if (mileageCtrl.text == '35.0') mileageCtrl.text = '15.0';
                                if (tankCtrl.text == '13.0') tankCtrl.text = '45.0';
                              });
                            }
                          },
                        ),
                  ...
                  const Text('Vehicle Name or Model', style: TextStyle(color: Colors.white70, fontSize: 12, fontWeight: FontWeight.w600)),
                  const SizedBox(height: 6),
                  TextField(
                    controller: nameCtrl,
                    style: const TextStyle(color: Colors.white, fontSize: 14),
                    decoration: InputDecoration(
                      hintText: customType == 'motorcycle' ? 'e.g. Royal Enfield Continental GT' : 'e.g. Mahindra Thar 4x4',
                  ...
                  const SizedBox(height: 14),
                  const Text('Fuel Type', style: TextStyle(color: Colors.white70, fontSize: 12, fontWeight: FontWeight.w600)),
                  const SizedBox(height: 6),
                  Wrap(
                    spacing: 6,
                    children: [
                      _dialogFuelChip('Petrol', 'petrol', customFuel, (f) => setDlgState(() => customFuel = f)),
                      if (customType == 'car')
                        _dialogFuelChip('Diesel', 'diesel', customFuel, (f) => setDlgState(() => customFuel = f)),
                      _dialogFuelChip('CNG', 'cng', customFuel, (f) => setDlgState(() => customFuel = f)),
                      _dialogFuelChip('EV', 'ev', customFuel, (f) => setDlgState(() => customFuel = f)),
                    ],
                  ),
                  const SizedBox(height: 14),
                  Row(
                    children: [
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text(customFuel == 'ev' ? 'Range (km)' : 'Mileage (km/L)', style: const TextStyle(color: Colors.white70, fontSize: 12, fontWeight: FontWeight.w600)),
                            const SizedBox(height: 6),
                            TextField(
                      ...
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text(customFuel == 'ev' ? 'Battery (kWh)' : 'Tank Size (L)', style: const TextStyle(color: Colors.white70, fontSize: 12, fontWeight: FontWeight.w600)),
                            const SizedBox(height: 6),
                            TextField(
                              controller: tankCtrl,
                              keyboardType: const TextInputType.numberWithOptions(decimal: true),
              ...
              ElevatedButton(
                ...
                onPressed: () {
                  final name = nameCtrl.text.trim().isNotEmpty
                      ? nameCtrl.text.trim()
                      : (customType == 'motorcycle' ? 'Custom Motorcycle' : 'Custom Car');
                  final mileage = double.tryParse(mileageCtrl.text) ?? (customType == 'motorcycle' ? 35.0 : 15.0);
                  final tank = double.tryParse(tankCtrl.text) ?? (customType == 'motorcycle' ? 13.0 : 45.0);
                  ...
                  final customVehicle = VehicleModel(
                    ...
                    bodyType: customType == 'motorcycle' ? 'Custom Bike' : 'Custom Vehicle',
                    isUserMileageOverride: true,
                ...
                child: const Text('Select Vehicle', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
              ),
            ],
          );
        },
  ...
  Widget _dialogFuelChip(String label, String value, String current, ValueChanged<String> onSelected) {
              ...
              Row(
                children: [
                  Container(
                    ...
                    child: Icon(_getHeaderIcon(), color: _getHeaderColor(), size: 22),
                  ),
                  const SizedBox(width: 12),
                  Expanded(
                    child: Column(
                  ...
                  TextButton.icon(
                    ...
                    icon: const Icon(Icons.add_rounded, color: Color(0xFF38BDF8), size: 16),
                    label: const Text('Custom', style: TextStyle(color: Color(0xFF38BDF8), fontSize: 12, fontWeight: FontWeight.bold)),
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    _categorySegment('Cars', 'car', Icons.directions_car_rounded),
                    _categorySegment('Bikes & Scooters', 'motorcycle', Icons.two_wheeler_rounded),
              ...
              Container(
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    prefixIcon: Icon(Icons.search, color: _getHeaderColor(), size: 20),
                  ...
                  child: Column(
                    children: [
                      ...
                      ElevatedButton(
                        ...
                        child: const Text('Confirm Vehicle Selection', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold)),
                      ),
                    ],
                  ),
                ),
          ...
          children: [
            ...
            Container(
              ...
              child: Icon(
                isBike
                    ? Icons.two_wheeler_rounded
                    : (v.fuelType == 'ev' ? Icons.bolt_rounded : Icons.directions_car_rounded),
                size: 18,
                color: isBike
                    ? const Color(0xFFF59E0B)
                    : (v.fuelType == 'ev' ? const Color(0xFFA78BFA) : const Color(0xFF60A5FA)),
              ),
            ),
            // Details
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Wrap(
                    ...
                    children: [
                      if (v.fuelType == 'ev') ...[
                        _specBadge(Icons.bolt, '${v.evRangeKm ?? 150} km Range', const Color(0xFF8B5CF6)),
                        if (v.batteryCapacityKwh != null)
                          _specBadge(Icons.battery_charging_full, '${v.batteryCapacityKwh} kWh', Colors.white60),
                      ] else ...[
                      ...
                      if (v.transmission != null)
                        _specBadge(
                          Icons.tune_rounded,
                          v.transmission!.contains('Auto') || v.transmission!.contains('AT') || v.transmission!.contains('CVT') ? 'AT' : 'MT',
                          Colors.white60,
                        ),
                    ],
                  ),
  ...
  Widget _brandChip(String label, String brandSearch) {
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: isSelected ? const Color(0xFF3B82F6).withOpacity(0.25) : const Color(0xFF1F2433),
          borderRadius: BorderRadius.circular(8),
