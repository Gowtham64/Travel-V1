...
class VehicleModel {
  ...
  /// Primary catalog image served by Vahan Details.
  ///
  /// The catalog keeps this derived so backend/local records do not need to
  /// duplicate image URLs. If a record cannot be mapped confidently, the UI
  /// falls back to its vehicle icon.
  String? get sourceImageUrl {
    final urls = sourceImageUrls;
    return urls.isEmpty ? null : urls.first;
  }

  /// Candidate image URLs because the source catalog has legacy naming
  /// differences between brands and model pages.
  List<String> get sourceImageUrls {
    final brand = (brandName ?? _inferVehicleBrand(name)).trim();
    final displayName = (modelName ?? _stripBrand(name, brand)).trim();
    if (displayName.isEmpty || brand.isEmpty) return const [];

    final kind = type.toLowerCase() == 'motorcycle' ? 'Bikes' : 'Cars';
    final sourceBrand = _vahanImageBrandFolder(brand);
    final brandFolder = Uri.encodeComponent(brand);
    final modelNames = <String>{
      displayName,
      displayName.replaceAll('.ev', ' EV').replaceAll('.EV', ' EV'),
      displayName.replaceAll('.', ''),
    };
    final urls = <String>[
      Uri.parse('${AppConfig.backendUrl}/api/vehicles/image').replace(
        queryParameters: {
          'type': type,
          'brand': brand,
          'model': displayName,
    ...
    for (final model in modelNames) {
      final sourceFolder = Uri.encodeComponent('$sourceBrand $model');
      final displayFolder = Uri.encodeComponent('$brand $model');
      final prefixes = <String>{
        '$brand-$sourceBrand $model',
        '$brand-$brand $model',
        '$brand-$model',
      };
      for (final prefix in prefixes) {
        ...
        urls.add(
          'https://restapi.vahandetails.com/public/Vahandetails_Images/'
          '$kind/$brandFolder/$sourceFolder/${encodedPrefix}-vahandetails-com1.webp',
        );
        urls.add(
          'https://restapi.vahandetails.com/public/Vahandetails_Images/'
          '$kind/$brandFolder/$displayFolder/${encodedPrefix}-vahandetails-com1.webp',
        );
      }
    }
  ...
  VehicleModel copyWith({
    ...
    return VehicleModel(
      ...
      isUserMileageOverride:
          isUserMileageOverride ?? this.isUserMileageOverride,
...
String _inferVehicleBrand(String vehicleName) {
  const brands = [
    'Maruti Suzuki',
    'Royal Enfield',
    'Mercedes-Benz',
  ...
  for (final brand in brands) {
    if (vehicleName.toLowerCase().startsWith(brand.toLowerCase())) {
      return brand;
    }
  }
  return '';
...
String _stripBrand(String vehicleName, String brand) {
  if (brand.isNotEmpty &&
      vehicleName.toLowerCase().startsWith(brand.toLowerCase())) {
    return vehicleName.substring(brand.length).trim();
  }
  return vehicleName;
}

String _vahanImageBrandFolder(String brand) {
