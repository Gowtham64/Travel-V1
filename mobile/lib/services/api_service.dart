...
class ApiService {
  ...
  ApiService({String? baseUrl}) : baseUrl = baseUrl ?? _prodBackend;
  ...
  Future<Map<String, String>> _authHeaders() async {
    final t = await AuthSession.instance.getValidAccessToken();
    return {
  ...
  String _accountErr(http.Response res) {
    ...
    if (res.statusCode == 401) return 'Session expired. Please log in again.';
    if (res.statusCode == 403) return 'Access denied (403).';
    if (res.statusCode == 404) return 'Requested item not found (404).';
    if (res.statusCode >= 500) return 'Server unavailable (${res.statusCode}). Please retry.';
    return 'Request failed (${res.statusCode})';
  }

  /// Offline account data is namespaced by the authenticated Supabase user.
  /// A network outage must never make User B see User A's cached records.
  String _localStoreKey(String path, {String? type}) {
    final userId = AuthSession.instance.user?.id;
    return 'voy_account_${userId ?? 'anonymous'}_${path}_${type ?? 'all'}';
  }

  String get _profileStoreKey =>
      'voy_cached_profile_${AuthSession.instance.user?.id ?? 'anonymous'}';

  Future<List<Map<String, dynamic>>> _loadLocalItems(String path,
      {String? type}) async {
    try {
  ...
  Future<void> _saveLocalItems(String path, List<Map<String, dynamic>> items,
      {String? type}) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      await prefs.setString(
          _localStoreKey(path, type: type), jsonEncode(items));
    } catch (_) {}
  }

  Future<List<Map<String, dynamic>>> accountList(String path,
    ...
    try {
      ...
      final uri = Uri.parse('$baseUrl/api/account/$path')
    ...
    try {
      ...
      return rows.map((r) {
        ...
        return VehicleModel(
          ...
          type:
              t.contains('bike') || t.contains('motor') ? 'motorcycle' : 'car',
  ...
  Future<Map<String, dynamic>> accountCreate(
    ...
    if (!AuthSession.instance.isAuthenticated) {
      throw ApiException('Sign in is required to save account data.');
    }
    try {
      final headers = await _authHeaders();
      final res = await http
          .post(Uri.parse('$baseUrl/api/account/$path'),
  ...
  Future<Map<String, dynamic>> accountUpdate(
      String path, String id, Map<String, dynamic> data) async {
    if (!AuthSession.instance.isAuthenticated) {
      throw ApiException('Sign in is required to update account data.');
    }
    try {
      final headers = await _authHeaders();
      final res = await http
          .patch(Uri.parse('$baseUrl/api/account/$path/$id'),
              headers: headers, body: jsonEncode(data))
          .timeout(const Duration(seconds: 8));
      if (res.statusCode == 200) {
        final item = (jsonDecode(res.body) as Map).cast<String, dynamic>();
  ...
  Future<void> accountDelete(String path, String id) async {
    if (!AuthSession.instance.isAuthenticated) {
      throw ApiException('Sign in is required to remove account data.');
    }
    try {
      final headers = await _authHeaders();
      await http
          .delete(Uri.parse('$baseUrl/api/account/$path/$id'),
              headers: headers)
          .timeout(const Duration(seconds: 8));
    } catch (_) {}
    // Local deletion
  ...
  Future<Map<String, dynamic>> getProfile() async {
    try {
      final headers = await _authHeaders();
      final res = await http
          .get(Uri.parse('$baseUrl/api/account/profile'),
              headers: headers)
          .timeout(const Duration(seconds: 8));
      if (res.statusCode == 200) {
        final profile = (jsonDecode(res.body) as Map).cast<String, dynamic>();
        final prefs = await SharedPreferences.getInstance();
        await prefs.setString(_profileStoreKey, jsonEncode(profile));
        return profile;
      }
    } catch (_) {}
    try {
      final prefs = await SharedPreferences.getInstance();
      final raw = prefs.getString(_profileStoreKey);
  ...
  Future<Map<String, dynamic>> putProfile(Map<String, dynamic> data) async {
    try {
      final headers = await _authHeaders();
      final res = await http
          .put(Uri.parse('$baseUrl/api/account/profile'),
              headers: headers, body: jsonEncode(data))
          .timeout(const Duration(seconds: 8));
      if (res.statusCode == 200) {
        final profile = (jsonDecode(res.body) as Map).cast<String, dynamic>();
    ...
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString(_profileStoreKey, jsonEncode(data));
    return data;
  }
  ...
  Future<Map<String, dynamic>> convertCurrency(
    ...
    final uri = Uri.parse('$baseUrl/api/currency/convert').replace(
  ...
  Future<GeoPoint> geocode(String address, {GeoPoint? near}) async {
    ...
    final response =
        await http.get(uri).timeout(const Duration(seconds: 60), onTimeout: () {
      throw ApiException(
          'Server is waking up (can take up to 60s). Please try again in a moment!');
    });
  ...
  Future<GeoPoint?> _geocodeWithMapbox(String address, {GeoPoint? near}) async {
    try {
      final uri = Uri.parse(
        ...
        'proximity':
            near != null ? '${near.lng},${near.lat}' : '78.9629,20.5937',
      });
  ...
  Future<({List<LatLng> path, double? lengthKm})> fetchTrekGeometry(
      String id) async {
    final uri = Uri.parse('$baseUrl/api/treks/geometry')
        .replace(queryParameters: {'id': id});
    final response =
        await http.get(uri).timeout(const Duration(seconds: 60), onTimeout: () {
      throw ApiException(
          'Trail is taking too long to load. Try again in a moment.');
    });
    if (response.statusCode != 200) {
    ...
    final path = (body['path'] as List? ?? [])
        .map((e) => LatLng(((e as Map)['lat'] as num).toDouble(),
  ...
  Future<TripPlan> planTrip({
    ...
    final response = await http
      ...
      throw ApiException(
          'Server is generating your plan (can take up to 90s). Please try again if it fails!');
    });

    if (response.statusCode != 200) {
      throw ApiException(
          'Trip planning failed (${response.statusCode}): ${response.body}');
    }

    var plan =
        TripPlan.fromJson(jsonDecode(response.body) as Map<String, dynamic>);
    ...
    if (fuelStops.isNotEmpty) {
      ...
      for (final s in fuelStops) {
        debugPrint(
            '[FUEL] Stop added to itinerary: ${s.name} at (${s.lat}, ${s.lng}), ${s.distanceFromStartKm} km');
      }
    }

    // Ensure tolls are calculated accurately from the actual route polyline and vehicle type
    if (plan.toll == null ||
        plan.toll!.tolls.isEmpty ||
        plan.toll!.isEstimated) {
  ...
  Future<Map<String, List<PlaceOfInterest>>> fetchPOIs({
    ...
    if (sampledCoords.length > 150) {
      ...
      if (sampledCoords.isEmpty ||
          sampledCoords.last != routeCoordinates.last) {
      ...
      if (response.statusCode == 200) {
        ...
        final res = placesJson.map(
          (key, value) => MapEntry(
            key,
            (value as List)
                .map((e) => PlaceOfInterest.fromJson(e as Map<String, dynamic>))
                .toList(),
          ),
        );
        if (res.values.any((l) => l.isNotEmpty)) {
          ...
          if (sampledCoords.isNotEmpty) {
            ...
            for (final list in res.values) {
              list.sort((a, b) {
                final distA = (a.lat - startPt.lat) * (a.lat - startPt.lat) +
                    (a.lng - startPt.lng) * (a.lng - startPt.lng);
                final distB = (b.lat - startPt.lat) * (b.lat - startPt.lat) +
                    (b.lng - startPt.lng) * (b.lng - startPt.lng);
            ...
            if (resp.statusCode == 200) {
              ...
              for (final f in features) {
                ...
                if (coords != null && coords.length >= 2) {
                  ...
                  // Clean up name
                  if (rawName.isEmpty ||
                      rawName.toLowerCase() == 'temple' ||
                      rawName.toLowerCase() == 'place_of_worship') {
                    if (cat == 'temple') {
                      rawName =
                          city != null ? 'Sri Temple ($city)' : 'Sri Temple';
                    } else if (cat == 'fuel') {
                      rawName = city != null
                          ? 'Fuel Station ($city)'
                          : 'Fuel Station';
                  ...
                  if (props['street'] != null)
                    addressParts.add(props['street'].toString());
                  ...
                  final key =
                      '$rawName-${lat.toStringAsFixed(3)}-${lng.toStringAsFixed(3)}';
                  if (!seenKeys.contains(key)) {
                    seenKeys.add(key);

                    final temple = (cat == 'temple')
                        ? TempleDatabase.findTemple(rawName)
                        : null;
                    ...
                    result[cat]?.add(
                      PlaceOfInterest(
                        ...
                        rating: temple?.rating ??
                            (isTemple
                                ? 4.7
                                : (cat == 'attraction' ? 4.6 : 4.4)),
                        reviewsCount:
                            temple?.reviewsCount ?? (isTemple ? 15000 : 8500),
                        deity: temple?.deity,
                        timing: temple?.timing ??
                            (isTemple
                                ? 'Opens 6:00 AM · Closes 8:30 PM'
                                : null),
                        highlights: temple?.highlights,
                        categoryType: isTemple
                            ? '🛕 Hindu temple'
                            : (cat == 'attraction'
                                ? '📍 Landmark / Attraction'
                                : (cat == 'viewpoint'
  ...
  Future<void> saveTrip({
        ...
        final inserted = await Supabase.instance.client
            ...
            .insert({
              ...
              'start_point': {
                'lat': start.lat,
                'lng': start.lng,
                if (start.name != null) 'name': start.name
              },
    ...
    if (response.statusCode != 200) {
      throw ApiException(
          'Saving trip failed (${response.statusCode}): ${response.body}');
    }
  }

  Future<void> updateTripStatus(
      String id, String newStatus, String token) async {
    try {
  ...
  Future<List<dynamic>> getSavedTrips(String token) async {
    ...
    final response = await http.get(
      ...
      throw ApiException(
          'Loading your trips is taking too long. Please try again.');
    });

    if (response.statusCode != 200) {
      throw ApiException(
          'Fetching trips failed (${response.statusCode}): ${response.body}');
    }
  ...
  Future<void> deleteTrip(String id, String token) async {
    ...
    try {
      ...
      if (response.statusCode != 200 && response.statusCode != 204) {
        debugPrint(
            'Backend deleteTrip note: (${response.statusCode}) ${response.body}');
      }
    } catch (e) {
  ...
  static const List<Map<String, dynamic>> _curatedPlaces = [
    ...
    {
      'name': 'Tirumala, Tirupati, Andhra Pradesh, India',
      'lat': 13.6833,
      'lng': 79.3500
    },
    ...
    {
      'name': 'Hubballi (Hubli), Karnataka, India',
      'lat': 15.3647,
      'lng': 75.1240
    },
    {
    ...
    {
      'name': 'Srikalahasti, Andhra Pradesh, India',
      'lat': 13.7498,
      'lng': 79.6984
    },
    {
      'name': 'Kanipakam, Andhra Pradesh, India',
      'lat': 13.2798,
      'lng': 79.0347
    },
    ...
    {
      'name': 'Kukke Subramanya, Karnataka, India',
      'lat': 12.6631,
      'lng': 75.6148
    },
    {
  ...
  Future<List<Map<String, dynamic>>> autocompletePlaces(String query) async {
    ...
    for (final p in _curatedPlaces) {
      ...
      if (lowerName.startsWith(q) ||
          primaryName == q ||
          (q.length >= 3 && primaryName.startsWith(q))) {
        addResult(
            name, (p['lat'] as num).toDouble(), (p['lng'] as num).toDouble());
        curatedCount++;
        if (curatedCount >= 3)
          break; // Allow room for Photon / Mapbox suggestions
      }
    }
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        final userSearchedCountry = q.contains('philippine') ||
            q.contains('burkina') ||
            q.contains('indonesia') ||
            q.contains('cameroon');
        if (!userSearchedCountry) {
          features.sort((a, b) {
            final propsA =
                (a as Map)['properties'] as Map<String, dynamic>? ?? {};
            final propsB =
                (b as Map)['properties'] as Map<String, dynamic>? ?? {};
            final ccA = (propsA['countrycode'] ?? '').toString().toUpperCase();
            final ccB = (propsB['countrycode'] ?? '').toString().toUpperCase();
            final isDomA = ccA == 'IN' ||
                (propsA['country'] ?? '').toString().toLowerCase() == 'india';
            final isDomB = ccB == 'IN' ||
                (propsB['country'] ?? '').toString().toLowerCase() == 'india';
        ...
        for (final f in features) {
          ...
          if (props != null && coords.length >= 2) {
            ...
            final labelParts = [name, city, state, country]
  ...
  Future<String?> reverseGeocode(double lat, double lng) async {
    ...
    try {
      final uri =
          Uri.parse('$baseUrl/api/trip/reverse-geocode?lat=$lat&lng=$lng');
    ...
    try {
      ...
      final res = await http.get(osmUri, headers: {
        'User-Agent': 'VoyPlanTravelApp/1.0'
  ...
  List<Map<String, String>> _parseAiPlaces(http.Response response) {
    if (response.statusCode == 503) {
      throw ApiException(
          'AI isn\'t enabled yet. Ask the server admin to set GEMINI_API_KEY.');
    }
    if (response.statusCode != 200) {
  ...
  Future<List<Map<String, String>>> aiRecommendStops({
    ...
    final response = await http
        .post(
          ...
          headers: {'Content-Type': 'application/json'},
          body:
              jsonEncode({'start': start, 'end': end, 'waypoints': waypoints}),
        )
  ...
  Future<List<Map<String, dynamic>>> aiBuildItinerary({
    ...
    try {
      final response = await http
          .post(
            ...
            body: jsonEncode({
              ...
              if (startTime != null)
                'startTime': TripDateTime.to24Hour(startTime),
              if (startDateTime != null)
                'startDateTime': startDateTime
              else if (startDate != null && startTime != null)
                'startDateTime':
                    '$startDate ${TripDateTime.to24Hour(startTime)}',
      ...
      if (response.statusCode == 200) {
        final list = (jsonDecode(response.body) as Map<String, dynamic>)['days']
                as List? ??
            [];
        final parsed =
  ...
  Future<
    ...
    try {
      final response = await http
          .post(
            ...
            body: jsonEncode({
              ...
              if (categoryPriorities.isNotEmpty)
      ...
      if (response.statusCode == 200) {
        ...
        final budget = body['budget'] != null
            ? TripBudget.fromJson(
                (body['budget'] as Map).cast<String, dynamic>())
            : null;
        ...
        final navRoute = body['navigationRoute'] != null
            ? NavigationRoute.fromJson(
                (body['navigationRoute'] as Map).cast<String, dynamic>())
            : null;
        final tripPlan = body['tripPlan'] != null
            ? TripPlan.fromJson(
                (body['tripPlan'] as Map).cast<String, dynamic>())
            : null;
        final routeVersion = (body['routeVersion'] as num?)?.toInt() ?? 1;
        final totalDist =
            route?.distanceKm ?? (body['totalDistanceKm'] as num?)?.toDouble();
        final totalDur =
            route?.durationMin ?? (body['totalDurationMin'] as num?)?.toInt();
      ...
      debugPrint(
          '[API] smart-itinerary remote call failed, using high-precision local engine: $e\n$st');
    }
    ...
    return (
      ...
      tripType: tripType,
      searchRadiusKm: searchRadiusKm,
      placesFoundCount: fb.days.fold(0,
          (sum, d) => sum + d.blocks.where((b) => b.type == 'activity').length),
      ...
      if (response.statusCode == 200) {
        ...
        final tripPlan = body['tripPlan'] != null
            ? TripPlan.fromJson(
                (body['tripPlan'] as Map).cast<String, dynamic>())
            : null;
        final totalDist = route?.distanceKm ??
            ((body['totalDistanceKm'] as num?)?.toDouble() ?? 0.0);
        final totalDur = route?.durationMin ??
            ((body['totalDurationMin'] as num?)?.toInt() ?? 0);
        final rVer =
            (body['routeVersion'] as num?)?.toInt() ?? (routeVersion + 1);
  ...
  Future<
    ...
    if (response.statusCode != 200) {
      throw ApiException(
          'Route calculation failed (${response.statusCode}): ${response.body}');
    }

    final body = jsonDecode(response.body) as Map<String, dynamic>;
    final route =
        RouteInfo.fromJson((body['route'] as Map).cast<String, dynamic>());
    final navRoute = NavigationRoute.fromJson(
        (body['navigationRoute'] as Map).cast<String, dynamic>());
    final budget = body['budget'] != null
  ...
  List<Map<String, dynamic>> _generateFallbackBuildItinerary({
    ...
    final startPart = h24 < 12
        ? 'Morning'
        : (h24 < 17 ? 'Afternoon' : (h24 < 21 ? 'Evening' : 'Night'));
    ...
    for (int d = 1; d <= total; d++) {
      ...
      res.add({
        'day': d,
        'title': isFirst
            ? 'Journey to $end & Exploration'
            : (isLast ? 'Farewell $end & Return' : '$end Highlights & Culture'),
        'activities': [
          {
            'part': isFirst ? startPart : 'Morning',
            'time': isFirst ? canonicalStart24 : '08:00',
            'title': isFirst
                ? 'Drive from $start to $end'
                : 'Breakfast & Morning Sights in $end',
            'note': isFirst
                ? 'Scenic drive along the highway'
                : 'Fresh regional breakfast and sightseeing visit',
          },
          {
            'part': 'Afternoon',
            'time': '13:00',
          ...
          },
          {
            ...
            'time': '17:30',
            'title': isLast
                ? 'Sunset Return Drive to $start'
                : 'Sunset Viewpoint & Local Bazaar',
            'note': isLast
                ? 'Comfortable highway return journey'
                : 'Tea, photography and local shopping',
          },
          {
            'part': 'Night',
            'time': '20:30',
            'title':
                isLast ? 'Arrival back at $start' : 'Dinner & Rest in $end',
            'note': isLast
                ? 'Safe return home'
                : 'Relaxing dinner and evening city ambiance',
          },
        ],
      });
    }
  ...
  ({
    ...
    double totalDistanceKm,
    int totalDurationMin
  }) _generateFallbackSmartItinerary({
    required dynamic destination,
    dynamic startLocation = '',
    String tripType = 'around',
    ...
    final text = '$destName $preferences $customPreferences ${places.join(" ")}'
        .toLowerCase();
    ...
    final parenAlias =
        parenMatch != null ? parenMatch.group(1)!.toLowerCase().trim() : '';
    final searchTerms = [cleanCity.toLowerCase(), parenAlias]
        .where((s) => s.length >= 3)
        .toList();
    ...
    final allAttractions = <_FallbackAttraction>[
      ...
      const _FallbackAttraction(
          ...
          durationMin: 75,
          highlight:
              'Illuminated marble corridors, golden gopuram & sacred Pushkarini',
          categories: [
      ...
      const _FallbackAttraction(
          ...
          durationMin: 90,
          highlight:
              'Jet skiing, boat rides, camel rides & sunset photography along Arabian Sea',
          categories: [
            'Beaches',
      ...
      const _FallbackAttraction(
          ...
          durationMin: 90,
          highlight:
              'Tranquil beach with dense pine canopy, ferry crossing & walking trails',
          categories: [
            'Beaches',
            'Nature & Forests',
            'Viewpoints & Scenic Places'
          ]),
      const _FallbackAttraction(
          ...
          durationMin: 75,
          highlight:
              'Historic hill shrine with natural mountain springs & Pandava caves',
          categories: [
            'Temples & Religious Places',
            'Historical & Heritage Places',
            'Hills & Mountains'
      ...
      const _FallbackAttraction(
          ...
          durationMin: 60,
          highlight:
              'Magnificent Sistine Chapel-style ceiling frescoes by Italian Jesuit Bro. Moscheni',
          categories: [
            'Historical & Heritage Places',
            'Cultural Places',
            'Monuments & Landmarks'
      ...
      const _FallbackAttraction(
          ...
          durationMin: 150,
          highlight:
              'Safari zoo, heritage artisanal village, lake boating & 3D planetarium',
          categories: [
      ...
      const _FallbackAttraction(
          ...
          durationMin: 60,
          highlight:
              'Tipu Sultan 1784 naval watchtower overlooking river mouth & boat jetty',
          categories: [
            'Forts & Palaces',
            'Historical & Heritage Places',
            'Rivers, Lakes & Waterfalls'
      ...
      const _FallbackAttraction(
          ...
          durationMin: 75,
          highlight:
              'Dramatic large monolithic sea boulders & panoramic sunset viewpoint',
          categories: [
            'Beaches',
            'Viewpoints & Scenic Places',
            'Instagrammable / Photography Spots'
      ...
      const _FallbackAttraction(
          ...
          durationMin: 60,
          highlight:
              'Panoramic 360-degree ocean lookout atop rocky coastal lighthouse hill',
          categories: [
            'Viewpoints & Scenic Places',
            'Beaches',
            'Monuments & Landmarks'
      ...
      const _FallbackAttraction(
          ...
          durationMin: 90,
          highlight:
              'Sacred river island sanctum surrounded by rushing streams of Nandini river',
          categories: [
            'Temples & Religious Places',
            'Rivers, Lakes & Waterfalls'
          ]),
      const _FallbackAttraction(
          ...
          durationMin: 90,
          highlight:
              'Historic 13th-century Madhvacharya matha, golden ratha & holy pond',
          categories: [
            'Temples & Religious Places',
            'Cultural Places',
      ...
      const _FallbackAttraction(
          ...
          durationMin: 180,
          highlight:
              'Scenic ferry ride to million-year-old columnar basalt rock formations',
          categories: [
            'Beaches',
            'Nature & Forests',
            'Instagrammable / Photography Spots',
      ...
      const _FallbackAttraction(
          ...
          durationMin: 150,
          highlight:
              'Sweeping Western Ghats mountain vistas, cool mist and hilltop temple',
          categories: [
            'Hills & Mountains',
            'Viewpoints & Scenic Places',
            'Instagrammable / Photography Spots'
      ...
      const _FallbackAttraction(
          ...
          durationMin: 120,
          highlight:
              'Dramatic mountain pass, historic caves and origin of Indian coffee',
          categories: [
            'Hills & Mountains',
            'Cultural Places',
            'Historical & Heritage Places'
      ...
      const _FallbackAttraction(
          ...
          city: 'Near Chikmagalur',
          rating: '4.8',
          durationMin: 180,
          highlight:
              'Exciting 4x4 jungle ride & trek through coffee plantations to roaring falls',
          categories: [
            'Rivers, Lakes & Waterfalls',
            'Nature & Forests',
            'Hills & Mountains'
      ...
      const _FallbackAttraction(
          ...
          durationMin: 90,
          highlight:
              'Thrilling cliffside walking trail with 360-degree green valley views',
          categories: [
            'Viewpoints & Scenic Places',
            'Hills & Mountains',
            'Instagrammable / Photography Spots'
      ...
      const _FallbackAttraction(
          ...
          durationMin: 120,
          highlight:
              'Speed boating in emerald reservoir surrounded by misty Banasura hills',
          categories: [
            'Famous Bridges / Dams',
            'Rivers, Lakes & Waterfalls',
            'Viewpoints & Scenic Places'
      ...
      const _FallbackAttraction(
          name: 'Edakkal Caves & Ancient Stone Age Carvings',
          city: 'Wayanad',
          rating: '4.7',
          durationMin: 120,
          highlight:
              'Scenic uphill mountain trek to prehistoric rock engravings & valley view',
          categories: [
            'Historical & Heritage Places',
            'Hills & Mountains',
            'Cultural Places'
      ...
      const _FallbackAttraction(
          ...
          city: 'Meppadi, Wayanad',
          rating: '4.7',
          durationMin: 120,
          highlight:
              'Walk through tea plantations and lush evergreen forest to natural pool',
          categories: [
            'Rivers, Lakes & Waterfalls',
            'Nature & Forests',
            'Instagrammable / Photography Spots'
      ...
      const _FallbackAttraction(
          ...
          city: 'Lakkidi, Wayanad',
          rating: '4.6',
          durationMin: 60,
          highlight:
              'Dramatic 700m high cliff edge looking over winding Thamarassery Churam',
      ...
      const _FallbackAttraction(
          ...
          durationMin: 90,
          highlight:
              'Iconic 1924 basalt arch monument overlooking Mumbai harbour',
          categories: [
            'Historical & Heritage Places',
            'Monuments & Landmarks',
            'Famous / Must-Visit Places'
      ...
      const _FallbackAttraction(
          name: 'Bandra-Worli Sea Link & Sea View Promenade',
          city: 'Mumbai',
          rating: '4.8',
          durationMin: 60,
          highlight:
              'Spectacular 8-lane cable-stayed bridge spanning the Arabian Sea',
          categories: [
            'Famous Bridges / Dams',
            'Viewpoints & Scenic Places',
            'Instagrammable / Photography Spots'
      ...
      const _FallbackAttraction(
          ...
          durationMin: 90,
          highlight:
              'Curved 3.6 km coastal promenade with sweeping sunset Arabian sea views',
          categories: [
            'Viewpoints & Scenic Places',
            'Famous City Attractions',
            'Instagrammable / Photography Spots'
      ...
      const _FallbackAttraction(
          ...
          durationMin: 60,
          highlight:
              'UNESCO World Heritage Victorian Gothic architecture & illuminated facade',
          categories: [
            'Historical & Heritage Places',
            'Monuments & Landmarks',
            'Famous / Must-Visit Places'
      ...
      const _FallbackAttraction(
          ...
          durationMin: 180,
          highlight:
              'Rock-cut 5th-century cave temples of Lord Shiva reached by scenic boat ride',
          categories: [
            'Historical & Heritage Places',
            'Cultural Places',
            'Rivers, Lakes & Waterfalls'
      ...
      const _FallbackAttraction(
          ...
          durationMin: 180,
          highlight:
              'Protected green forest reserve with ancient Buddhist caves and tiger safari',
          categories: [
            'Wildlife & National Parks',
            'Nature & Forests',
            'Hills & Mountains'
      ...
      const _FallbackAttraction(
          name: 'Juhu Beach & Street Food Boulevard',
          city: 'Mumbai',
          rating: '4.6',
          durationMin: 90,
          highlight:
              'Sunset beach with famous Pav Bhaji, Bhel Puri and Arabian Sea breezes',
          categories: [
            'Beaches',
            'Famous Markets & Local Places',
            'Famous City Attractions'
          ]),
      const _FallbackAttraction(
          name: 'Colaba Causeway & Heritage Art District',
          city: 'Mumbai',
          rating: '4.7',
          durationMin: 90,
          highlight:
              'Bustling artisanal shopping bazaar, vintage cafes and art galleries',
          categories: ['Famous Markets & Local Places', 'Cultural Places']),
      const _FallbackAttraction(
          name: 'Malabar Hill & Hanging Gardens',
          city: 'Mumbai',
          rating: '4.6',
          durationMin: 60,
          highlight:
              'Terraced hilltop gardens with panoramic views of the city skyline',
          categories: [
            'Hills & Mountains',
            'Viewpoints & Scenic Places',
            'Nature & Forests'
      ...
      const _FallbackAttraction(
          ...
          durationMin: 75,
          highlight:
              'Historic 1801 gold-plated sanctum dedicated to Lord Ganesha',
          categories: ['Temples & Religious Places']),
      ...
      const _FallbackAttraction(
          ...
          durationMin: 150,
          highlight:
              'Golden Throne, stained glass Kalyana Mantapa & illuminated royal durbar',
          categories: [
            'Forts & Palaces',
            'Historical & Heritage Places',
            'Famous / Must-Visit Places'
      ...
      const _FallbackAttraction(
          ...
          durationMin: 120,
          highlight:
              'Terraced botanical gardens and synchronized musical dancing fountains',
          categories: [
            'Nature & Forests',
            'Rivers, Lakes & Waterfalls',
            'Famous City Attractions'
      ...
      const _FallbackAttraction(
          ...
          durationMin: 150,
          highlight:
              'Historic 1892 sanctuary with giraffes, big cats & exotic birds',
          categories: [
            'Wildlife & National Parks',
            'Nature & Forests',
            'Famous / Must-Visit Places'
      ...
      const _FallbackAttraction(
          name: 'KRS Dam (Krishna Raja Sagara Dam)',
          city: 'Mysuru',
          rating: '4.7',
          durationMin: 90,
          highlight:
              'Majestic Kaveri river reservoir dam gates & illuminated walkways',
          categories: [
            'Famous Bridges / Dams',
            'Rivers, Lakes & Waterfalls',
            'Viewpoints & Scenic Places'
      ...
      const _FallbackAttraction(
          name: 'Karanji Lake & Walk-Through Aviary',
          city: 'Mysuru',
          rating: '4.6',
          durationMin: 75,
          highlight:
              'Scenic lake boating, butterfly park & India\'s largest walk-through aviary',
          categories: [
            'Rivers, Lakes & Waterfalls',
            'Nature & Forests',
            'Viewpoints & Scenic Places'
      ...
      const _FallbackAttraction(
          ...
          durationMin: 75,
          highlight:
              'Vibrant 100-year-old market with pure sandalwood, silk & Mysore Pak',
          categories: [
            'Famous Markets & Local Places',
            'Cultural Places',
            'Famous City Attractions'
      ...
      const _FallbackAttraction(
          ...
          durationMin: 45,
          highlight:
              'Twin 175ft spires, stained glass French windows & subterranean crypts',
          categories: [
            'Monuments & Landmarks',
            'Historical & Heritage Places',
            'Cultural Places'
      ...
      const _FallbackAttraction(
          ...
          durationMin: 90,
          highlight:
              'Historic royal gallery housing original Raja Ravi Varma oil masterpieces',
          categories: [
            'Cultural Places',
            'Forts & Palaces',
            'Historical & Heritage Places'
      ...
      const _FallbackAttraction(
          name: 'Sri Chamundeshwari Temple & Monolithic Nandi',
      ...
      const _FallbackAttraction(
          ...
          durationMin: 90,
          highlight:
              'Kaveri river boat safari viewing migratory storks, pelicans & crocodiles',
          categories: [
            'Wildlife & National Parks',
            'Rivers, Lakes & Waterfalls',
            'Nature & Forests'
      ...
      // --- Coorg ---
      const _FallbackAttraction(
          name: 'Abbey Falls & Hanging Bridge',
          city: 'Madikeri, Coorg',
          rating: '4.7',
          durationMin: 75,
          highlight:
              '70ft roaring waterfall surrounded by private coffee and spice estates',
          categories: [
            'Rivers, Lakes & Waterfalls',
            'Nature & Forests',
            'Instagrammable / Photography Spots'
      ...
      const _FallbackAttraction(
          ...
          durationMin: 60,
          highlight:
              'Panoramic sunset view over Western Ghats mist-covered valleys',
          categories: [
            'Viewpoints & Scenic Places',
            'Hills & Mountains',
            'Instagrammable / Photography Spots'
      ...
      const _FallbackAttraction(
          name: 'Dubare Elephant Camp & River Crossing',
          city: 'Kushalnagar, Coorg',
          rating: '4.7',
          durationMin: 120,
          highlight:
              'Interactive elephant care, river boating & white water rafting',
          categories: [
            'Wildlife & National Parks',
            'Rivers, Lakes & Waterfalls',
            'Nature & Forests'
      ...
      const _FallbackAttraction(
          ...
          durationMin: 90,
          highlight:
              '40ft gold-plated Buddha statues, ornate Tibetan murals & peace bell',
          categories: [
            'Cultural Places',
            'Temples & Religious Places',
            'Famous / Must-Visit Places'
      ...
      const _FallbackAttraction(
          ...
          durationMin: 150,
          highlight:
              'Off-road 4x4 adventure through clouds to 4000ft high mountain ridge',
          categories: [
            'Hills & Mountains',
            'Viewpoints & Scenic Places',
            'Instagrammable / Photography Spots'
      ...
      const _FallbackAttraction(
          name: 'Nagarhole National Park (Kabini Safari)',
          city: 'Coorg / Kabini',
          rating: '4.8',
          durationMin: 180,
          highlight:
              'Forest jeep safari spotting wild Asian elephants, leopards & tigers',
          categories: ['Wildlife & National Parks', 'Nature & Forests']),

      // --- Ooty ---
      const _FallbackAttraction(
          name: 'Ooty Botanical Gardens & Victorian Glass House',
          city: 'Ooty',
          rating: '4.7',
          durationMin: 90,
          highlight:
              'Historic 55-acre garden with 20-million-year-old fossilized tree',
      ...
      const _FallbackAttraction(
          ...
          durationMin: 75,
          highlight:
              'Highest peak in Nilgiris (8650ft) with 360-degree telescope observatory',
          categories: [
            'Hills & Mountains',
            'Viewpoints & Scenic Places',
            'Instagrammable / Photography Spots'
      ...
      const _FallbackAttraction(
          name: 'Ooty Lake & Boating Promenade',
          city: 'Ooty',
          rating: '4.6',
          durationMin: 75,
          highlight:
              'Picturesque mountain lake surrounded by towering eucalyptus groves',
          categories: [
            'Rivers, Lakes & Waterfalls',
            'Famous City Attractions'
          ]),
      const _FallbackAttraction(
          name: 'Pykara Waterfalls & Lake Speed Boating',
          city: 'Near Ooty',
          rating: '4.7',
          durationMin: 90,
          highlight:
              'Pristine tiered waterfalls and tranquil speed boat lake cruises',
          categories: [
            'Rivers, Lakes & Waterfalls',
            'Nature & Forests',
            'Instagrammable / Photography Spots'
      ...
      const _FallbackAttraction(
          ...
          durationMin: 120,
          highlight:
              'UNESCO Heritage vintage steam train winding through mist and tunnels',
          categories: [
            'Historical & Heritage Places',
            'Famous / Must-Visit Places',
            'Cultural Places'
      ...
      const _FallbackAttraction(
          name: 'Calangute & Baga Beach Strip',
          city: 'Goa',
          rating: '4.7',
          durationMin: 120,
          highlight:
              'Vibrant golden sand shoreline, water sports shacks & sunset sea breeze',
          categories: [
            'Beaches',
            'Famous / Must-Visit Places',
            'Instagrammable / Photography Spots'
      ...
      const _FallbackAttraction(
          ...
          durationMin: 90,
          highlight:
              '17th-century Portuguese coastal fortress overlooking Arabian Sea',
          categories: [
            'Forts & Palaces',
            'Historical & Heritage Places',
            'Viewpoints & Scenic Places'
      ...
      const _FallbackAttraction(
          ...
          durationMin: 210,
          highlight:
              'Four-tiered milky mountain waterfall in lush Bhagwan Mahavir Sanctuary',
          categories: [
            'Rivers, Lakes & Waterfalls',
            'Nature & Forests',
            'Hills & Mountains'
      ...
      const _FallbackAttraction(
          ...
          durationMin: 75,
          highlight:
              'UNESCO World Heritage baroque cathedral & sacred relics of St. Francis Xavier',
          categories: [
            'Historical & Heritage Places',
            'Cultural Places',
            'Monuments & Landmarks'
      ...
      const _FallbackAttraction(
          ...
          durationMin: 120,
          highlight:
              'Crescent-shaped tranquil white sand bay with dolphin boat tours',
          categories: [
            'Beaches',
            'Viewpoints & Scenic Places',
            'Instagrammable / Photography Spots'
      ...
      const _FallbackAttraction(
          name: 'Chapora Fort (Dil Chahta Hai Vista)',
          city: 'Goa',
          rating: '4.7',
          durationMin: 60,
          highlight:
              'Dramatic cliffside ramparts with panoramic sunset view of Vagator Beach',
          categories: [
            'Forts & Palaces',
            'Viewpoints & Scenic Places',
            'Instagrammable / Photography Spots'
      ...
      const _FallbackAttraction(
          name: 'Anjuna Flea Market & Beach Promenade',
          city: 'Goa',
          rating: '4.6',
          durationMin: 90,
          highlight:
              'Bustling bohemian flea market with handicrafts, spices, live music & beach cafes',
          categories: ['Famous Markets & Local Places', 'Cultural Places']),
      ...
      const _FallbackAttraction(
          ...
          durationMin: 120,
          highlight:
              'Wodeyar Tudor-style fortified turrets, royal ballrooms & manicured lawns',
          categories: [
            'Forts & Palaces',
            'Historical & Heritage Places',
            'Famous / Must-Visit Places'
      ...
      const _FallbackAttraction(
          ...
          durationMin: 120,
          highlight:
              'Victorian Glass House, lotus lake and 3000-million-year peninsular rock',
          categories: [
            'Nature & Forests',
            'Famous City Attractions',
            'Instagrammable / Photography Spots'
      ...
      const _FallbackAttraction(
          ...
          durationMin: 180,
          highlight:
              'Grand wildlife reserve, bus safari and butterfly conservatory',
          categories: ['Wildlife & National Parks', 'Nature & Forests']),
      const _FallbackAttraction(
          name: 'Cubbon Park & Vidhana Soudha Architecture',
          city: 'Bengaluru',
          rating: '4.7',
          durationMin: 75,
          highlight:
              'Neo-Dravidian architecture and shaded colonial park promenades',
          categories: [
            'Monuments & Landmarks',
            'Nature & Forests',
            'Famous City Attractions'
      ...
      const _FallbackAttraction(
          name: 'Nandi Hills & Sunrise Cloud-Sea Vista',
          city: 'Near Bengaluru',
          rating: '4.8',
          durationMin: 150,
          highlight: '4851ft mountain fortress with sea of clouds sunrise view',
          categories: [
            'Hills & Mountains',
            'Viewpoints & Scenic Places',
            'Instagrammable / Photography Spots'
      ...
      const _FallbackAttraction(
          name: 'Commercial Street & Brigade Road Bazaar',
          city: 'Bengaluru',
          rating: '4.6',
          durationMin: 90,
          highlight:
              'Bustling shopping lanes with artisanal silk, crafts and cafes',
          categories: [
            'Famous Markets & Local Places',
            'Famous City Attractions'
          ]),
    ...
    var cityCandidates = allAttractions
        .where((a) => searchTerms.any(
            (st) => a.city.toLowerCase().contains(st) || text.contains(st)))
        .toList();

    // Strict Category Filtering
    final hasCategoryFilter = selectedCategories.isNotEmpty;
    final templesAllowed = !hasCategoryFilter ||
        selectedCategories.contains('Temples & Religious Places');

    var pool = <_FallbackAttraction>[];

    if (hasCategoryFilter) {
      pool = cityCandidates
          .where((a) => a.categories.any((c) => selectedCategories.contains(c)))
          .toList();

      if (!templesAllowed) {
        pool = pool
            .where((a) => !a.categories.contains('Temples & Religious Places'))
            .toList();
      }
      ...
      pool.sort((a, b) {
        final aMust =
            a.categories.any((c) => categoryPriorities[c] == 'must_visit');
        final bMust =
            b.categories.any((c) => categoryPriorities[c] == 'must_visit');
    ...
    _FallbackAttraction synthesizeAttraction(
        String city, String cat, int count) {
      final c = count > 0 ? count : 1;
      if (cat == 'Temples & Religious Places') {
        final names = [
          '$city Sacred Heritage Shrine',
          '$city Hilltop Spiritual Sanctum',
          '$city Ancient Cultural Temple'
        ];
        return _FallbackAttraction(
            name: names[(c - 1) % names.length],
            city: city,
            rating: '4.8',
            durationMin: 75,
            highlight:
                'Historic sanctum, spiritual heritage and traditional stone architecture',
            categories: const [
              'Temples & Religious Places',
              'Historical & Heritage Places'
            ]);
      ...
      if (cat == 'Beaches') {
        final names = [
          '$city Sunset Beach & Coastal Walkway',
          '$city Golden Sands Promenade',
          '$city Coastal Bay & Water Sports Beach'
        ];
        return _FallbackAttraction(
            name: names[(c - 1) % names.length],
            city: city,
            ...
            durationMin: 90,
            highlight:
                'Golden sand coastline, sea breeze and evening coastal sunset',
            categories: const [
              'Beaches',
              'Viewpoints & Scenic Places',
              'Instagrammable / Photography Spots'
      ...
      if (cat == 'Hills & Mountains') {
        final names = [
          '$city Misty Mountain Peak & Ridge Lookout',
          '$city Valley Viewpoint & Mountain Trail',
          '$city Cloud-Capped Hilltop Ridge'
        ];
        return _FallbackAttraction(
            name: names[(c - 1) % names.length],
            city: city,
            ...
            durationMin: 120,
            highlight:
                'High altitude clouds, mountain breeze and valley vistas',
            categories: const [
              'Hills & Mountains',
              'Viewpoints & Scenic Places'
            ]);
      ...
      if (cat == 'Rivers, Lakes & Waterfalls') {
        final names = [
          '$city Scenic Waterfalls & Cascades',
          '$city Lakefront Promenade & Boating',
          '$city Natural River Gorge & Falls'
        ];
        return _FallbackAttraction(
            name: names[(c - 1) % names.length],
            city: city,
            ...
            durationMin: 90,
            highlight:
                'Cascading natural waterfalls and serene water promenade',
            categories: const [
              'Rivers, Lakes & Waterfalls',
              'Nature & Forests'
            ]);
      ...
      if (cat == 'Viewpoints & Scenic Places') {
        final names = [
          '$city Panoramic Sunset Valley Viewpoint',
          '$city Skyline Lookout & Promenade',
          '$city Scenic Landscape Vista'
        ];
        return _FallbackAttraction(
            name: names[(c - 1) % names.length],
            city: city,
            ...
            durationMin: 60,
            highlight:
                'Breathtaking 360-degree landscape and golden hour sunset vista',
            categories: const [
              'Viewpoints & Scenic Places',
              'Instagrammable / Photography Spots'
            ]);
      ...
      if (cat == 'Forts & Palaces') {
        final names = [
          '$city Historic Royal Fort & Bastion',
          '$city Heritage Palace & Royal Grounds',
          '$city Ancient Citadel & Courtyard'
        ];
        return _FallbackAttraction(
            name: names[(c - 1) % names.length],
            city: city,
            ...
            durationMin: 120,
            highlight:
                'Grand royal architecture and fortified courtyard grounds',
            categories: const [
              'Forts & Palaces',
              'Historical & Heritage Places'
            ]);
      ...
      if (cat == 'Wildlife & National Parks') {
        final names = [
          '$city Wildlife Sanctuary & Safari',
          '$city Nature Reserve & Fauna Park',
          '$city Botanical Bird Sanctuary'
        ];
        return _FallbackAttraction(
            name: names[(c - 1) % names.length],
            city: city,
            ...
            durationMin: 150,
            highlight:
                'Protected natural fauna habitat and guided flora safari',
            categories: const [
              'Wildlife & National Parks',
              'Nature & Forests'
            ]);
      ...
      if (cat == 'Nature & Forests') {
        final names = [
          '$city Lush Botanical Gardens & Tree Park',
          '$city Forest Reserve & Canopy Trail',
          '$city Green Valley Eco Park'
        ];
        return _FallbackAttraction(
            name: names[(c - 1) % names.length],
            city: city,
            ...
            durationMin: 90,
            highlight:
                'Scenic canopy walkways, rare flora and peaceful nature trails',
            categories: const [
              'Nature & Forests',
              'Rivers, Lakes & Waterfalls'
            ]);
      ...
      if (cat == 'Famous Markets & Local Places') {
        final names = [
          '$city Traditional Artisan Bazaar',
          '$city Heritage Spice & Craft Market',
          '$city Local Food & Souvenir Street'
        ];
        return _FallbackAttraction(
            name: names[(c - 1) % names.length],
            city: city,
            ...
            durationMin: 75,
            highlight:
                'Vibrant local market with regional delicacies, handicrafts and spices',
            categories: const [
              'Famous Markets & Local Places',
              'Cultural Places'
            ]);
      ...
      return _FallbackAttraction(
          name: '$city Iconic Heritage Landmark $c',
          city: city,
          rating: '4.8',
          durationMin: 90,
          highlight:
              'Regional landmark, photography spot and cultural heritage',
          categories: [cat.isNotEmpty ? cat : 'Famous / Must-Visit Places']);
    }
    ...
    _FallbackAttraction getNextAttraction([String timeSlot = 'any']) {
      ...
      if (hasCategoryFilter && selectedCategories.isNotEmpty) {
        final aligned =
            selectedCategories.where((c) => preferred.contains(c)).toList();
        if (aligned.isNotEmpty) {
          targetCategory = aligned[slotCounter % aligned.length];
        } else {
          targetCategory =
              selectedCategories[slotCounter % selectedCategories.length];
        }
      }
      slotCounter++;
      ...
      if (targetCategory != null) {
        for (final t in pool) {
          if (!usedAttractionNames.contains(t.name) &&
              t.categories.contains(targetCategory)) {
            chosen = t;
            break;
          }
        }
      ...
      if (chosen == null && preferred.isNotEmpty) {
        for (final t in pool) {
          if (!usedAttractionNames.contains(t.name) &&
              t.categories.any((c) => preferred.contains(c))) {
            chosen = t;
            break;
          }
        }
    ...
    if (!isOverseas) {
      if (pair.contains('bengaluru') || pair.contains('bangalore')) {
        ...
        if (pair.contains('maddur') || pair.contains('mandya'))
          estimatedKm = 1020.0;
        else if (pair.contains('pune'))
          estimatedKm = 150.0;
        else if (pair.contains('goa'))
          estimatedKm = 585.0;
        else if (pair.contains('lonavala'))
          estimatedKm = 85.0;
        else if (pair.contains('shirdi'))
          estimatedKm = 240.0;
        else if (pair.contains('mahabaleshwar'))
          estimatedKm = 260.0;
    ...
    String getEmoji(String cat) {
      ...
      if (cat.contains('Wildlife') || cat.contains('National Park'))
        return '🐘';
    ...
    ({List<String> categories, String match}) resolveCats(
        _FallbackAttraction t) {
      if (selectedCategories.isEmpty) {
        return (
          categories: t.categories,
          match: t.categories.isNotEmpty ? t.categories.first : 'Famous Places'
        );
      }
      final matches =
          t.categories.where((c) => selectedCategories.contains(c)).toList();
      if (matches.isNotEmpty) {
        return (categories: matches, match: matches.first);
      }
      return (
        categories: t.categories,
        match: t.categories.isNotEmpty
            ? t.categories.first
            : selectedCategories.first
      );
    }
    ...
    final coffeeHighway = VenueDatabase.getBestVenue(
        destination: destName,
        type: 'coffee',
        highwayRoute: '$startName $destName');
    final breakfastVenue =
        VenueDatabase.getBestVenue(destination: destName, type: 'breakfast');
    final lunchVenue =
        VenueDatabase.getBestVenue(destination: destName, type: 'lunch');
    final dinnerVenue =
        VenueDatabase.getBestVenue(destination: destName, type: 'dinner');
    final hotelVenue =
        VenueDatabase.getBestVenue(destination: destName, type: 'hotel');
    ...
    for (int d = 1; d <= total; d++) {
      ...
      if (isFirst) {
        ...
        final driveMin = isOverseas
            ? totalDriveMin
            : (totalDriveMin > 360 ? 300 : totalDriveMin);
        blocks.add(TimelineBlock(
          start: formatMin(cur),
          end: formatMin(cur + driveMin),
          type: 'travel',
          title: isOverseas
              ? 'Flight / Travel from $startName to $destName'
              : 'Drive from $startName to $destName',
          place: destName,
        ...
        final t1Dur = t1.durationMin > 90
            ? 75
            : (t1.durationMin > 0 ? t1.durationMin : 75);
        ...
        blocks.add(TimelineBlock(
          ...
          whyIncluded:
              'Matches your selected ${res1.match} preference along the route.',
        ));
        cur += t1Dur + 15; // 15 min buffer to reach sunset spot

        // Evening Sunset Sightseeing 2
        final t2 = getNextAttraction('sunset');
        final t2Dur = t2.durationMin > 90
            ? 75
            : (t2.durationMin > 0 ? t2.durationMin : 75);
        ...
        blocks.add(TimelineBlock(
          ...
          whyIncluded:
              'Matches your selected ${res2.match} preference along the route.',
        ));
        cur += t2Dur;
        ...
        final t1Duration = t1.durationMin > 150
            ? 120
            : (t1.durationMin > 0 ? t1.durationMin : 90);
        final res1 = resolveCats(t1);
        final emoji1 = getEmoji(res1.match);
        blocks.add(TimelineBlock(
          start: '09:15 AM',
        ...
        final t2Duration = t2.durationMin > 120
            ? 105
            : (t2.durationMin > 0 ? t2.durationMin : 90);
        final res2 = resolveCats(t2);
        final emoji2 = getEmoji(res2.match);
        blocks.add(TimelineBlock(
          start: '02:00 PM',
        ...
        blocks.add(TimelineBlock(
          ...
          whyIncluded:
              'Matches your selected ${res3.match} preference along the route.',
        ));
        ...
        final t1 = getNextAttraction('morning');
        final t1Duration = t1.durationMin > 90
            ? 75
            : (t1.durationMin > 0 ? t1.durationMin : 75);
        final res1 = resolveCats(t1);
        final emoji1 = getEmoji(res1.match);
        ...
        final t2 = getNextAttraction('afternoon');
        final t2Duration = t2.durationMin > 90
            ? 75
            : (t2.durationMin > 0 ? t2.durationMin : 75);
        final res2 = resolveCats(t2);
        final emoji2 = getEmoji(res2.match);
        ...
        blocks.add(TimelineBlock(
          ...
          type: 'return',
          title: isOverseas
              ? 'Return Flight back to $startName'
              : 'Return Drive back to $startName',
          place: startName,
      ...
      daysList.add(SmartDay(
        ...
        title: isFirst
            ...
            : (isLast
                ? 'Farewell $destName & Return'
                : 'Full Exploration of $destName'),
        blocks: blocks,
      ));
    }

    final eff =
        (fuelEfficiency != null && fuelEfficiency > 0) ? fuelEfficiency : 15.0;
    ...
    if (estimatedKm > 50.0) {
      ...
      final pair = '$startName $destName'.toLowerCase();
      if ((pair.contains('bengaluru') || pair.contains('bangalore')) &&
          (pair.contains('mysore') || pair.contains('mysuru'))) {
        tollCost = 320 *
            2; // Bengaluru-Mysuru Expressway round-trip (Kaniminike + Gananguru)
      } else if ((pair.contains('bengaluru') || pair.contains('bangalore')) &&
          pair.contains('chennai')) {
        ...
        tollCost = 320 * 2; // Mumbai-Pune Expressway round-trip
          ...
          (pair.contains('agra') || pair.contains('lucknow'))) {
    ...
    final stayCost =
        (total > 1 ? (total - 1) : 0) * 2200 * ((travellers / 2).ceil());
    const int bufferCost = 1000;
    final int grandTotal =
        (fuelCost + tollCost + foodCost + stayCost + bufferCost).toInt();
  ...
  Future<
    ...
    if (response.statusCode == 503) {
      throw ApiException(
          "AI isn't enabled yet. Ask the server admin to configure an AI key.");
    }
    if (response.statusCode != 200) {
  ...
  Future<String> aiAsk({
    ...
    final response = await http
        .post(
          ...
          headers: {'Content-Type': 'application/json'},
          body: jsonEncode(
              {'question': question, if (context != null) 'context': context}),
        )
  ...
  Future<({String shareId, String shareUrl, Map<String, dynamic> trip})>
      createTripShare(Map<String, dynamic> tripData) async {
    try {
      final response = await http
          .post(
      ...
      if (response.statusCode == 200) {
        ...
        return (
          shareId: b['shareId'] as String? ??
              'voy_${DateTime.now().millisecondsSinceEpoch}',
