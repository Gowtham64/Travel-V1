import CURATED_PLACES from "../data/curatedPlaces.json";
...
export const ACTIVE_MODEL = getActiveModel();
...
async function generate(prompt: string, opts: any = {}) {
  const provider = getProvider();
  const key = activeKey();
...
async function geminiGenerate(prompt: string, opts: any, key: string) {
  const modelToUse = opts.model || getActiveModel();
  const url = `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(modelToUse)}:generateContent?key=${key}`;
  const body: any = { contents: [{ role: "user", parts: [{ text: prompt }] }], generationConfig: { temperature: 0.7 } };
...
async function openaiCompatGenerate(prompt: string, opts: any, key: string) {
  const provider = getProvider();
  const base = provider === "groq" ? "https://api.groq.com/openai/v1" : "https://openrouter.ai/api/v1";
  const model = opts.model || (provider === "groq" ? (process.env.GROQ_MODEL || "openai/gpt-oss-20b") : (process.env.OPENROUTER_MODEL || "meta-llama/llama-3.3-70b-instruct:free"));
  const messages: any[] = [];
...
async function searchPlaces({ query, near }) {
  ...
  const fallback = [];
  const CURATED_TEMPLES = (CURATED_PLACES as any[]).filter((p: any) => p.category === 'temple' || (p.categories && p.categories.includes('temple')));
  for (const t of CURATED_TEMPLES) {
    const tName = (t.name || '').toLowerCase();
    const tCity = (t.city || '').toLowerCase();
    const tDeity = (t.deity || t.description || '').toLowerCase();
    if (tName.includes(qClean) || tDeity.includes(qClean) || tCity.includes(qClean) || qClean.includes(tName)) {
      fallback.push({
        name: t.name,
        area: t.city,
        why: `🛕 ${t.deity || t.name} · ⭐ ${t.rating || 4.5} · ${t.highlight || t.description || ''}`
      });
    }
  }
...
export async function listModels() {
  ...
  if (!res.ok) return [];
  const data: any = await res.json();
  return (data?.data || []).map((m: any) => m.id).slice(0, 60);
}
  ...
  if (Array.isArray(CURATED_PLACES)) {
    for (const p of CURATED_PLACES) {
      ...
      if ((pName && str.includes(pName)) || (pCity && str.includes(pCity))) {
        ...
        if ((p as any).latitude && (p as any).longitude) return { lat: (p as any).latitude, lng: (p as any).longitude };
      }
    }
  }
  return null;
 ...
 */
async function rankCandidatesWithAI({ candidates = [], destination = "", origin = "", maxStops = 6, preferences = "" }: any) {
  if (!candidates.length) return [];
  const destName = destination && typeof destination === "object" ? destination.name : String(destination || "");
  const destId = destination && typeof destination === "object" ? (destination.placeId || "") : "";
  const origName = origin && typeof origin === "object" ? origin.name : String(origin || "");
