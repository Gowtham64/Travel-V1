import { isReturnToOrigin } from "../utils/tripType";
 ...
 */
function calculateRouteTolls(start: any, end: any, vehicleKey: string = "car", routeCoordinates: any = null, options: any = {}) {
 ...
 */
async function getTollEstimate(start: any, end: any, vehicleKey: string = "car", routeCoordinates: any = null, options: any = {}, customApiKey?: string) {
  const apiKey = customApiKey || options?.tollGuruApiKey || "";
  const tollGuruVehicle = VEHICLE_TYPES[vehicleKey as keyof typeof VEHICLE_TYPES] || VEHICLE_TYPES.car;
  ...
  if (apiKey) {
    try {
      ...
      if (res.ok) {
        ...
        const route = responseData.routes && responseData.routes[0];
        ...
        return {
          ...
          currency: responseData.summary?.currency || "INR",
          totalAmount: totalTripToll,
          fastagTollCost: totalTripToll,
    ...
    console.warn("TollGuru API skipped (falling back to NHAI spatial database):", err?.message || err);
  }
  }
