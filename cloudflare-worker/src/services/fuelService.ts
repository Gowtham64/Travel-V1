 ...
 */
function resolveLocation({ locationName = '', countryCode = 'IN', lat, lng, latitude, longitude }: { locationName?: string; countryCode?: string; lat?: any; lng?: any; latitude?: any; longitude?: any } = {}) {
  if (lat === undefined && latitude !== undefined) lat = latitude;
  if (lng === undefined && longitude !== undefined) lng = longitude;
  ...
  if (!stateKey) {
    for (const [sKey, sObj] of Object.entries(countryData.states as Record<string, any>)) {
      if (norm.includes(sKey) || norm.includes(normalizeText(sObj.name))) {
        stateKey = sKey;
        break;
      }
      if (sObj.cities) {
        for (const [cKey] of Object.entries(sObj.cities as Record<string, any>)) {
  ...
  resolvedState = (countryData.states as any)[stateKey] || (countryData.states as any).karnataka || Object.values(countryData.states)[0];
 ...
 */
function getFuelPrices({ locationName = '', countryCode = 'IN', lat, lng, fuelType = 'petrol' }: { locationName?: string; countryCode?: string; lat?: any; lng?: any; fuelType?: string } = {}) {
  try {
